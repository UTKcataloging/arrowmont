...
class Settings:
    def __init__(self):
        self.pdf_output = "/home/mark/metadata/Arrowmont/original_data/pdfs/curriculum_docs"
        self.metadata_output = "/home/mark/metadata/Arrowmont/cleaned_data/mods/curriculum_docs"
...
class Record:
    ...
    def create_metadata_record(self, unique_id, metadata_output):
        ...
        title = f"<titleInfo><title>{self.data[0]}: {self.data[1]}</title></titleInfo>\n"
        subject = f"<abstract>{self.data[4]}</abstract>\n"
        author = "<name><namePart>Glenn Bogart</namePart><role><roleTerm authority='marcrelators' " \
                 "valueURI='http://id.loc.gov/vocabulary/relators/cre'>Creator</roleTerm></role></name>\n<name>" \
                 "<namePart>Valerie Dixon</namePart><role><roleTerm authority='marcrelators' " \
                 "valueURI='http://id.loc.gov/vocabulary/relators/cre'>Creator</roleTerm></role></name>\n<name>" \
                 "<namePart>Dave Ballentine</namePart><role><roleTerm authority='marcrelators' " \
                 "valueURI='http://id.loc.gov/vocabulary/relators/cre'>Creator</roleTerm></role></name>\n"
        ...
        identifier = f"<identifier type='filename'>{self.data[2].replace('http://www.lib.utk.edu/arrowmont/Curriculum/', '')}</identifier>\n"
        ...
        note = f'<note displayLabel="Grade level">{self.data[3]}</note>'
...
if __name__ == "__main__":
    my_set = Set("/home/mark/metadata/Arrowmont/original_data/csvs/curriculum-documents.csv")
